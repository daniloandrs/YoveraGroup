...
class SwalService {
    ...
    constructor () {
        ...
        let openAlert = () => {
            swal.modal('show');
            swal.on('hide.bs.modal', () => {
                if (config.methodClosed)
                    config.methodClosed();  
                swal.off('hide.bs.modal');
            });
        }

        let setProperties = (message,type, callbackAccept = null, callbackClosed = null,confirm = false) => {
            config.type = type;
            config.message = message;
            config.methodAccept = callbackAccept ? callbackAccept : undefined;
            config.methodClosed = callbackClosed ? callbackClosed : undefined;
            config.confirm = confirm;
            openAlert();
        } 

        this.error   = (message,callbackAccept = null,callbackClosed = null) => setProperties(message,'danger',callbackAccept,callbackClosed,false);

        this.success = (message,callbackAccept = null,callbackClosed = null) =>setProperties(message,'success',callbackAccept,callbackClosed,false);

        this.info    = (message,callbackAccept = null,callbackClosed = null) => setProperties(message,'info',callbackAccept,callbackClosed,false);

        this.confirm = (message, callbackAccept = null, callbackClosed = null) => setProperties(message,'info',callbackAccept,callbackClosed,true);

        // modal open 

        this.openModal =  (name , callback = undefined ) => {
            $('#openModal_'+name).modal('show');

            $('#openModal_'+name).on("hidden.bs.modal", function () {
...
let BreadcrumbComponent = () => {
    return {
        template:
            ...
            <nav class="navbar navbar-expand-md navbar-theme mb-3 no-content breadcrumb">
    ...
    return {
        template:
            ...
            <div class="md-form md-outline">
                <i ng:if="$ctrl.icon !== undefined " style="cursor:pointer" ng:click="$ctrl.iconEvent();" class="{{$ctrl.icon}} prefix"></i>
                <input  
...
//component <input-area></input-area>
        ...
        template:
            ...
            <div class="dn-input-area">
                ...
                <div class="md-form md-outline">
                    ...
                    <textarea 
                        id="{{ $ctrl.serial_key + '_' + $ctrl.name }}"
                        cols="{{$ctrl.cols ? $ctrl.cols : $ctrl.default_cols}}" rows="{{$ctrl.rows ? $ctrl.rows : $ctrl.default_rows}}"
...
let InputFileComponent  = () => {
    return {
            ...
            <div class="dn-uploaderfile" ng:init="onInit();">
                    ...
                    <div style="padding-bottom: 30px;" class="area-input bg-white label-dropzone" ng:class="{ 'label-full': options.multiple }" >
                        ...
                        <div class="card col-md-3 item-dropzone" ng:repeat="one in list__files" ng:class="{ 'row-error': one.validation.error }"">
                            ...
                            <button class="btn clean-dropzone btn-sm" ng:click="remove__event($index, one.attrs)">
                                <span>Eliminar </span> <i class="fa fa-trash fa-2x"></i>
                            </button>
                ...
                <div ng:if="!options.multiple" class="text-center">
                    <div class="text-center">
                        ...
                        font-size: 14px;
                        font-weight: bold;" ng:bind="$ctrl.title ? $ctrl.title : 'Subir Archivo' "></label>
                    </div>
                    <div class="area-input bg-white fix-col justify-content-center" ng:class="{ 'label-error': file__current.validation.error && !$ctrl.model,'label-success': file__current.validation.success && $ctrl.model}">
                        ...
                        <div class="hover-area-input fix-col justify-content-center">
                            <label for="{{$ctrl.nameModel}}" class="btn btn-info btn-sm">
                        ...
                        <button class="btn clean btn-sm" ng:click="clean__event();" ng:if="!validateClean();">
        ...
        controller: function ($scope,cApp,$rootScope, swal = 'swal.service') {
            ...
            scope.$watch('$ctrl.clearDefault',before => {
                if(before) {
...
angular
    ...
    .component('inputArea',InputAreaComponent())
